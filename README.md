...
1. Внести изменения версии в файл `VERSION`. Этот файл автоматически подставляет номер версии в tag Docker-образа и в параметр tag в values.yaml Helm chart.
2. Внести необходимые изменения в папке приложения app.
3. Выполнить `git push main dev` для отправки изменений в удалённый репозиторий.
4. Запустить выполнение pipeline в Jenkins.
5. Открыть страничку `stage.hitmouz.com`, чтобы проверить результаты.

*Для правильной работы pipeline в Jenkins в ветке `main` необходимо выполнить следующие шаги:*
1. Выполнить merge ветки main следующими командами `git checkout main`, потом `git merge dev`.
2. Зайти в pipeline в Jenkins и на этапе `Deploy to GKE prod` подтвердить выполнение.
3. Открыть страничку `prod.hitmouz.com`, чтобы проверить результаты.
...
![Screenshot_81](https://github.com/hitmouz/diplom/assets/95587607/ccf502bd-0323-4149-badb-386bf6259e3e)
